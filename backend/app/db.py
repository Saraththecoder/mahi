...
class MockCursor:
    ...
    def sort(self, key, direction=-1):
        try:
            self.data.sort(key=lambda x: x.get(key) if x.get(key) is not None else datetime.utcnow(), reverse=(direction==-1))
        except Exception:
            pass
        return self
...
class MockCollection:
    ...
    async def insert_one(self, document):
        if self.name not in IN_MEMORY_DB:
            IN_MEMORY_DB[self.name] = []
        IN_MEMORY_DB[self.name].append(document)
        return document
        
    def find(self, query=None):
        data = IN_MEMORY_DB.get(self.name, [])
...
# Default Seed Data for In-Memory Fallback
DEFAULT_SCHEMES = [
    {
        "_id": "pm-kisan",
        ...
        "eligibility_translated": {
            "te": ["సాగు చేయదగిన భూమి కలిగి ఉన్న చిన్న మరియు సన్నకారు రైతు కుటుంబాలు.", "భూమి పట్టా వారి పేరు మీద నమోదై ఉండాలి."],
            "hi": ["सभी छोटे और सीमांत भूमिधारक किसान परिवार जिनके पास कृषि योग्य भूमि है।", "भूमि स्वामित्व उनके नाम पर पंजीकृत होना चाहिए।"]
        },
        "benefits": [
        ...
        "benefits_translated": {
            "te": ["ఏడాదికి రూ. 6,000 ఆర్థిక లబ్ధి.", "ప్రతి 4 నెలలకు ఒకసారి రూ. 2,000 చొప్పున మూడు విడతల్లో నేరుగా రైతుల బ్యాంక్ ఖాతాల్లో జమ చేయబడుతుంది."],
            "hi": ["प्रति वर्ष 6,000 रुपये का वित्तीय लाभ।", "हर 4 महीने में सीधे किसानों के बैंक खातों में 2,000 रुपये की तीन समान किस्तों में भुगतान किया जाता है।"]
        },
        "required_documents": [
        ...
        "required_documents_translated": {
            "te": ["భారతీయ పౌరసత్వ ధృవీకరణ పత్రం", "భూమి యాజమాన్య పత్రాలు (పట్టాదారు పాస్బుక్)", "ఆధార్ కార్డ్", "బ్యాంక్ ఖాతా వివరాలు"],
            "hi": ["नागरिकता प्रमाण पत्र", "भूमि स्वामित्व दस्तावेज (खतौनी/पट्टा)", "आधार कार्ड", "बैंक खाता विवरण"]
        },
    ...
    },
    {
        ...
        "eligibility_translated": {
            "te": ["నోటిఫైడ్ ప్రాంతాలలో నోటిఫైడ్ పంటలు పండించే కౌలుదారులు మరియు భాగస్వామ్య రైతులతో సహా అందరూ రైతులు."],
            "hi": ["अधिसूचित क्षेत्रों में अधिसूचित फसलें उगाने वाले बंटाईदार और काश्तकार किसानों सहित सभी किसान।"]
        },
        "benefits": [
        ...
        "benefits_translated": {
            "te": ["పంట నష్టానికి సమగ్ర బీమా రక్షణ.", "తక్కువ ప్రీమియం రేట్లు: ఖరీఫ్ పంటలకు 2.0%, రబీ పంటలకు 1.5% మరియు వాణిజ్య/తోట పంటలకు 5.0%."],
            "hi": ["फसल नुकसान के लिए व्यापक जोखिम बीमा।", "समान रूप से कम प्रीमियम दरें: खरीफ फसलों के लिए 2.0%, रबी फसलों के लिए 1.5% और वाणिज्यिक/बागवानी फसलों के लिए 5.0%।"]
        },
        "required_documents": [
            "Aadhaar Card",
        ...
        "required_documents_translated": {
            "te": ["ఆధార్ కార్డ్", "భూమి రికార్డులు (అడంగల్ / 1B)", "స్థానిక రెవెన్యూ/వ్యవసాయ అధికారి జారీ చేసిన పంట సాగు ధృవీకరణ పత్రం", "బ్యాంక్ పాస్‌బుక్ కాపీ"],
            "hi": ["आधार कार्ड", "भूमि अभिलेख (खसरा/खतौनी)", "स्थानीय राजस्व/कृषि अधिकारी द्वारा जारी बुवाई प्रमाण पत्र", "बैंक पासबुक की प्रति"]
        },
    ...
    },
    {
        ...
        "eligibility_translated": {
            "te": ["సొంత భూమి సాగు చేసుకునే రైతులు అందరూ.", "కౌలు రైతులు మరియు భాగస్వామ్య సాగుదారులు.", "రైతుల స్వయం సహాయక సంఘాలు (SHGs)."],
            "hi": ["सभी मालिक काश्तकार किसान।", "बंटाईदार और पट्टाधारक किसान।", "किसानों के स्वयं सहायता समूह (SHG) या संयुक्त देयता समूह (JLG)।"]
        },
        "benefits": [
        ...
        "benefits_translated": {
            "te": ["సకాలంలో రుణం తిరిగి చెల్లిస్తే కేవలం 4% వడ్డీ రేటు.", "ఆధార్ కనెక్టివిటీ తో 1.6 లక్షల లోపు రుణాలకు ఎలాంటి హామీ అవసరం లేదు.", "రూ. 50,000 వరకు ప్రమాద బీమా రక్షణ."],
            "hi": ["समय पर पुनर्भुगतान करने पर प्रति वर्ष 4% की रियायती ब्याज दर।", "1.6 लाख रुपये तक के ऋण के लिए किसी सुरक्षा (संपार्श्विक) की आवश्यकता नहीं है।", "50,000 रुपये तक का दुर्घटना बीमा कवरेज शामिल है।"]
        },
        "required_documents": [
        ...
        "required_documents_translated": {
            "te": ["పూర్తి చేసిన దరఖాస్తు ఫారమ్", "గుర్తింపు కార్డు (ఆధార్, ఓటర్ ఐడీ)", "రెవెన్యూ అధికారి ధృవీకరించిన సాగు భూమి పత్రాలు", "పాస్‌పోర్ట్ సైజు ఫోటో"],
            "hi": ["पूर्ण भरा हुआ आवेदन पत्र", "पहचान प्रमाण (आधार कार्ड, मतदाता पहचान पत्र)", "स्थानीय राजस्व अधिकारी द्वारा सत्यापित कृषि भूमि के दस्तावेज", "पासपोर्ट आकार फोटो"]
        },
    ...
    },
    {
        ...
        "eligibility_translated": {
            "te": ["ఆంధ్రప్రదేశ్ నివాసితులైన రైతులు.", "ఎస్సీ, ఎస్టీ, బీసీ మరియు మైనారిటీ వర్గాలకు చెందిన కౌలు రైతులు కూడా అర్హులు."],
            "hi": ["आंध्र प्रदेश के निवासी किसान।", "इसमें एससी, एसटी, ओबीसी और अल्पसंख्यक काश्तकार (किराएदार) किसान शामिल हैं।"]
        },
        "benefits": [
        ...
        "benefits_translated": {
            "te": ["సంవత్సరానికి రూ. 13,500 పెట్టుబడి సహాయం (పీఎం-కిసాన్ వాటాతో కలిపి).", "వైఎస్సార్ జలకళ కింద ఉచితంగా బోర్ వెల్స్ తవ్వించడం.", "ఉచిత పంట బీమా రక్షణ మరియు వడ్డీ లేని రుణాలు."],
            "hi": ["प्रति वर्ष 13,500 रुपये की वित्तीय सहायता (पीएम-किसान हिस्से सहित)।", "वाईएसआर जल कला के तहत मुफ्त बोरवेल ड्रिलिंग।", "मुफ्त फसल बीमा कवरेज और शून्य-ब्याज ऋण।"]
        },
        "required_documents": [
            "Aadhaar Card",
        ...
        "required_documents_translated": {
            "te": ["ఆధార్ కార్డ్", "పట్టాదారు పాస్బుక్", "కౌలు ఒప్పంద పత్రం (కౌలు రైతులకు)", "ఆధార్‌తో లింక్ అయిన బ్యాంక్ ఖాతా వివరాలు"],
            "hi": ["आधार कार्ड", "भूमि पट्टाकार पासबुक", "पट्टा समझौता पत्र (काश्तकारों के लिए)", "आधार से जुड़ा बैंक खाता विवरण"]
        },
...
# Seed defaults to in-memory db immediately
IN_MEMORY_DB["government_schemes"] = DEFAULT_SCHEMES
IN_MEMORY_DB["agriculture_centers"] = DEFAULT_CENTERS

async def connect_to_mongo():
    logger.info("Connecting to MongoDB...")
    try:
        # Lower selection timeout to 1.5 seconds for instant fallback check
        ...
        logger.warning("FastAPI backend is starting in LOCAL IN-MEMORY MOCK database mode.")

async def close_mongo_connection():
    if db_instance.is_active and db_instance.client:
        logger.info("Closing MongoDB connection...")
        db_instance.client.close()
        logger.info("MongoDB connection closed.")

def get_collection(name: str):
    if not db_instance.is_active or db_instance.db is None:
        # Return our highly responsive mock collection layer
